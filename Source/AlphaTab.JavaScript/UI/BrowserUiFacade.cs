...
namespace AlphaTab.UI
{
    internal class BrowserUiFacade : IUiFacade<object>
    {
        ...
        public IAlphaSynth CreateWorkerPlayer()
        {
            ...
            {
                player.Ready += () => { ((AlphaTabApi)_api).LoadSoundFontFromUrl(_api.Settings.Player.SoundFontFile); };
            }
        ...
        {
            var scrollElement = Browser.Document.QuerySelector(_api.Settings.Player.ScrollElement);
            var nodeName = scrollElement.NodeName.ToLowerCase();
            if (nodeName == "html" || nodeName == "body")
            {
